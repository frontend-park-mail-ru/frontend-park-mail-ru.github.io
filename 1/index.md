...
1. [Знакомство, инструменты, настройка окружения](#1-1)
    * [Полезные ресурсы](#1-1-links)
    * [Лекция](#1-1-lecture)
    * [Практика](#1-1-practice)
    * [Домашнее задание](#1-1-hw)
    * [Техническое задание](#1-1-spec)
2. [Архитектура веб-приложений](#1-2)
    * [Полезные ресурсы](#1-2-links)
    * [Лекция](#1-2-lecture)
    * [Домашнее задание](#1-2-hw)
    * [Техническое задание](#1-2-spec)
...
Сегодняшняя лекция будет посвящена устройству современных веб-приложений и хорошим практикам. Начнем с одной из них — работой с глобальной областью видимости в JavaScript.
...
1. Самостоятельно изучить документацию Underscore, Backbone и RequireJS.
...
2. Модель игрока должна состоять из следующих атрибутов:

    - `name` – имя игрока (значение по умолчанию: `''`)
    - `score` – количество очков (значение по умолчанию: `0`)

3. Список игроков должено состоять из 10 разных записей и программно упорядочен по убыванию атрибута `score`.
4. Переходы между экранами веб-приложения должны отражаться в истории веб-браузера, таким образом, чтобы корректно работали кнопки Back и Forward. Для этой цели к адресу веб-страницы добавляется якорь (`#`):
