...
2. В HTML и CSS коде необходимо придерживаться методологии БЭМ.
3. Модель игрока должна состоять из следующих атрибутов:

    - `name` – имя игрока (значение по умолчанию: `''`)
    - `score` – количество очков (значение по умолчанию: `0`)

4. Список игроков должено состоять из 10 разных записей и программно упорядочен по убыванию атрибута `score`.
